...
public class GroupsTest
{
	...
	private static String name = "JUNIT_TEST", description = "JUNIT_TEST", message = "JUNIT_TEST_NACHRICHT";
	...
	/*
	 * @Test public void getGroupNotes() { try { int noteID =
	 * Notes.createNote("JUNIT_TEST_GROUPNOTE", "JUNIT_TEST_GROUPNOTE").getID();
	 * int groupID = Groups.createGroup(name, description).getID();
	 * System.out.println(noteID); System.out.println(groupID);
	 * Groups.addGroupNote(groupID, noteID);
	 * //assertEquals(Groups.getGroupNotes(groupID).get(0).getID(), noteID); }
	 * catch (JSONException | IOException e) { fail(e.getMessage()); } }
	 */
